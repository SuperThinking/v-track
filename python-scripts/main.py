...
def login(regno, password):
    ...
    br.set_cookiejar(cj)
    response = br.open('https://academicscc.vit.ac.in/student/stud_login.asp')
    html = response.read()
    soup = BeautifulSoup(html)
    im = soup.find('img', id='imgCaptcha')
    # print(im)
    image_response = br.open_novisit(im['src'])
    # print(Image.open(StringIO(image_response.read())))
    ...
    br.submit()
    print(regno, password, str(captcha))
    if (br.geturl() == 'https://academicscc.vit.ac.in/student/home.asp'):
        puts(colored.yellow("LOGIN SUCCESSFUL"))
        br.open(
            'https://academicscc.vit.ac.in/student/attn_report.asp?sem=WS&fmdt=01-Apr-2019&todt=17-Apr-2019'
        )
        response1 = br.open(
            'https://academicscc.vit.ac.in/student/attn_report.asp?sem=WS&fmdt=01-Apr-2019&todt=17-Apr-2019'
        )
...
def parseFacultyPage(br, facultyID):
    ...
    response = br.open(
        'https://vtop.vit.ac.in/student/official_detail_view.asp?empid=' +
        str(facultyID))
    html = response.read()
    soup = BeautifulSoup(html)
    ...
    openHours = []
    try:
        dayOne = infoTable[10].findAll('table')[0].findAll('tr')[1].findAll(
            'td')[0].text
        dayTwo = infoTable[10].findAll('table')[0].findAll('tr')[2].findAll(
            'td')[0].text
        startDayOne = infoTable[10].findAll('table')[0].findAll(
            'tr')[1].findAll('td')[1].text
        startDayTwo = infoTable[10].findAll('table')[0].findAll(
            'tr')[2].findAll('td')[1].text
        endDayOne = infoTable[10].findAll('table')[0].findAll('tr')[1].findAll(
            'td')[2].text
        endDayTwo = infoTable[10].findAll('table')[0].findAll('tr')[2].findAll(
            'td')[2].text
        openHours.append({
            'day': dayOne,
            'start_time': startDayOne,
            'end_time': endDayOne
        })
        openHours.append({
            'day': dayTwo,
            'start_time': startDayTwo,
            'end_time': endDayTwo
        })
    except IndexError:
        openHours = []
